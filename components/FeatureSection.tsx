...
export default function FeatureSection() {
  ...
  return (
    <section id="sourcing" className="py-16 md:py-24 px-4 sm:px-6 md:px-8 relative overflow-hidden bg-gradient-to-b from-coffee-espresso via-coffee-primary/30 to-coffee-espresso">
      {/* Background Decor */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] sm:w-[500px] sm:h-[500px] bg-coffee-accent/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-6 xl:gap-12 items-center">
          ...
          <div className="space-y-5 sm:space-y-8 order-2 lg:order-1">
            {features.filter(f => f.position === 'left').map((feature, i) => (
              <motion.div
                ...
                className="bg-coffee-secondary/60 backdrop-blur-sm p-4 sm:p-6 rounded-2xl border border-coffee-border/50 hover:border-coffee-accent/40 transition-colors duration-300"
              >
                <div className="flex items-center justify-between gap-4 mb-3">
                  <h3 className="text-lg sm:text-xl md:text-2xl font-playfair font-semibold text-coffee-text-primary">
                    {feature.title}
                  </h3>
                ...
                <p className="text-xs sm:text-sm text-coffee-text-secondary font-inter leading-relaxed">
                  {feature.description}
                </p>
              </motion.div>
            ))}
          ...
          >
            <div className="relative w-48 h-48 xs:w-56 xs:h-56 sm:w-80 sm:h-80 flex items-center justify-center">
              {/* Radial Blur Glow */}
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(79,156,143,0.25)_0%,transparent_70%)] blur-2xl" />
              
              {/* Inner Circle Frame */}
              <div className="absolute w-[150px] h-[150px] xs:w-[175px] xs:h-[175px] sm:w-[250px] sm:h-[250px] rounded-full bg-gradient-to-b from-[#1A0F0A]/95 to-[#2D1810]/95 border border-[#5A4034]/40 shadow-inner flex items-center justify-center overflow-hidden">
          ...
          <div className="space-y-5 sm:space-y-8 order-3">
